...
<div class="all">
    ...
    $search=$_POST["search"];
    $anzahl = $pdo->prepare("SELECT COUNT(*) FROM recipe_dish_type WHERE dish_type_id='2'");
    if ($anzahl -> execute()){
        while($row=$anzahl->fetch()){
            if ($row["COUNT(*)"]==1){
                echo "<div class='ueberschrift'>".$row["COUNT(*)"]. " Ergebnis wurde für die Menüart \"Dessert\" gefunden."."<br>"."</div>";
            }else{
                echo "<div class='ueberschrift'>".$row["COUNT(*)"]. " Ergebnisse wurden für die Menüart \"Dessert\" gefunden."."<br>"."</div>";
            }}}
    ?>
