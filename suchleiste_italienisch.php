...
<div class="all">
    <div class="main_wrapper">
        ...
        $search=$_POST["search"];
        $anzahl = $pdo->prepare("SELECT COUNT(*) FROM recipe_cuisine WHERE cuisine_id='1'");
        if ($anzahl -> execute()){
            while($row=$anzahl->fetch()){
                if ($row["COUNT(*)"]==1){
                    echo "<div class='ueberschrift'>".$row["COUNT(*)"]. " Ergebnis wurde für die Küche (Region) \"italienisch\" gefunden."."<br>"."</div>";
                }else{
                    echo "<div class='ueberschrift'>".$row["COUNT(*)"]. " Ergebnisse wurden für die Küche (Region) \"italienisch\" gefunden."."<br>"."</div>";
                }}}
        ?>
        ...
        <?php
        $statement=$pdo->prepare("SELECT recipe.id, title, description, level_name, timespan, filename, cuisine_id FROM recipe,level,timespan, picture, recipe_cuisine, cuisine WHERE cuisine.id='1' AND recipe_cuisine.cuisine_id=cuisine.id AND recipe_cuisine.recipe_id=recipe.id  AND recipe.level_id=level.id AND recipe.timespan_id=timespan.id AND picture.recipe_id=recipe.id GROUP BY recipe.id ORDER BY recipe.id DESC");
