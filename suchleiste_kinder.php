...
<head>
    ...
    <script type="text/javascript">
        function addToFavorites(row_id){
            ...
            window.location.href= str.substring(0, str.lastIndexOf("/"))+"/add_favorite.php?id="+row_id+"&backUrl=suchleiste_kinder.php";
        }

        function deleteFromFavorites(row_id){
            console.log("Deleting from favorites "+row_id);
            let str=window.location.href;
            window.location.href= str.substring(0, str.lastIndexOf("/"))+"/delete_favorite.php?id="+row_id+"&backUrl=suchleiste_kinder.php";
        }
    </script>
...
<div class="all">
    <div class="main_wrapper">
        ...
        $search=$_POST["search"];
        $anzahl = $pdo->prepare("SELECT COUNT(*) FROM recipe_category WHERE category_id='5'");
        if ($anzahl -> execute()){
            while($row=$anzahl->fetch()){
                if ($row["COUNT(*)"]==1){
                    echo "<div class='ueberschrift'>".$row["COUNT(*)"]. " Ergebnis wurde für die Sonderkategorie \"Kinder\" gefunden."."<br>"."</div>";
                }else{
                    echo "<div class='ueberschrift'>".$row["COUNT(*)"]. " Ergebnisse wurden für die Sonderkategorie \"Kinder\" gefunden."."<br>"."</div>";
                }}}
        ?>
        <div class="container_mittig">
            <div class="weite_suchleiste">
                <form action="suchleiste_do_kinder.php" method="post" enctype="multipart/form-data">
                    <div class="input-group mb-3">
                        <div class="input-group-prepend">
                            <button class="btn btn-light dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="height: 53px;margin-top: 1px;color: #FC7F16">Kinder</button>
