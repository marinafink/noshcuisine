...
<head>
    <meta charset="UTF-8">
    <title>Suchleiste - Nosh Cuisine</title>
    ...
    <script type="text/javascript">
        function addToFavorites(row_id){
            ...
            window.location.href= str.substring(0, str.lastIndexOf("/"))+"/add_favorite.php?id="+row_id+"&backUrl=suchleiste_histaminfrei.php";
        }

        function deleteFromFavorites(row_id){
            console.log("Deleting from favorites "+row_id);
            let str=window.location.href;
            window.location.href= str.substring(0, str.lastIndexOf("/"))+"/delete_favorite.php?id="+row_id+"&backUrl=suchleiste_histaminfrei.php";
        }
    </script>
...
<div class="all">
    <div class="main_wrapper">
        ...
        $search=$_POST["search"];
        $anzahl = $pdo->prepare("SELECT COUNT(*) FROM recipe_restriction WHERE restriction_id='5'");
        if ($anzahl -> execute()){
            while($row=$anzahl->fetch()){
                if ($row["COUNT(*)"]==1){
                    echo "<div class='ueberschrift'>".$row["COUNT(*)"]. " Ergebnis wurde für die Ernährungsweise \"histaminfrei\" gefunden."."<br>"."</div>";
                }else{
                    echo "<div class='ueberschrift'>".$row["COUNT(*)"]. " Ergebnisse wurden für die Ernährungsweise \"histaminfrei\" gefunden."."<br>"."</div>";
                }}}
        ?>
        <div class="container_mittig">
            <div class="weite_suchleiste">
                <form action="suchleiste_do_histaminfrei.php" method="post" enctype="multipart/form-data">
                    <div class="input-group mb-3">
                        <div class="input-group-prepend">
                            <button class="btn btn-light dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="height: 53px;margin-top: 1px;color: #FC7F16">histaminfrei</button>
        ...
        <?php
        $statement=$pdo->prepare("SELECT recipe.id, title, description, level_name, timespan, filename, restriction_id FROM recipe,level,timespan, picture, recipe_restriction, restriction WHERE restriction.id='5' AND recipe_restriction.restriction_id=restriction.id AND recipe_restriction.recipe_id=recipe.id  AND recipe.level_id=level.id AND recipe.timespan_id=timespan.id AND picture.recipe_id=recipe.id GROUP BY recipe.id ORDER BY recipe.id DESC");
