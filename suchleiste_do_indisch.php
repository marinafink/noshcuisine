...
<div class="all">
    <div class="main_wrapper">
        ...
        $search = $_POST["search"];
        $searchnew = "%$search%";
        $anzahl = $pdo->prepare("SELECT COUNT(*) FROM recipe, recipe_cuisine WHERE title LIKE :searchnew AND recipe_cuisine.cuisine_id='2' AND recipe.id=recipe_id ORDER BY RAND()");
        ...
        <!--AUSGABE ERGEBNISSE-->
        <?php
        $statement=$pdo->prepare("SELECT recipe.id, title, description, level_name, timespan, filename, cuisine_id FROM recipe,level,timespan, picture, recipe_cuisine, cuisine WHERE recipe_cuisine.cuisine_id='2' AND title LIKE :searchnew AND recipe_cuisine.cuisine_id=cuisine.id AND recipe_cuisine.recipe_id=recipe.id  AND recipe.level_id=level.id AND recipe.timespan_id=timespan.id AND picture.recipe_id=recipe.id GROUP BY recipe.id ORDER BY recipe.id DESC");
