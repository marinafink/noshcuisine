...
<div class="all">
    <div class="main_wrapper">
        ...
        $search = $_POST["search"];
        $searchnew = "%$search%";
        $anzahl = $pdo->prepare("SELECT COUNT(*) FROM recipe, recipe_dish_type WHERE title LIKE :searchnew AND recipe_dish_type.dish_type_id='10' AND recipe.id=recipe_id ORDER BY RAND()");
        ...
        <!--AUSGABE ERGEBNISSE-->
        <?php
        $statement=$pdo->prepare("SELECT recipe.id, title, description, level_name, timespan, filename, dish_type_id FROM recipe,level,timespan, picture, recipe_dish_type, dish_type WHERE recipe_dish_type.dish_type_id='10' AND title LIKE :searchnew AND recipe_dish_type.dish_type_id=dish_type.id AND recipe_dish_type.recipe_id=recipe.id  AND recipe.level_id=level.id AND recipe.timespan_id=timespan.id AND picture.recipe_id=recipe.id GROUP BY recipe.id ORDER BY recipe.id DESC");
