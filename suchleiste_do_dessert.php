...
<head>
    ...
    <script type="text/javascript">
        function addToFavorites(row_id){
            ...
            window.location.href= str.substring(0, str.lastIndexOf("/"))+"/add_favorite.php?id="+row_id+"&backUrl=suchleiste_dessert.php";
        }

        function deleteFromFavorites(row_id){
            console.log("Deleting from favorites "+row_id);
            let str=window.location.href;
            window.location.href= str.substring(0, str.lastIndexOf("/"))+"/delete_favorite.php?id="+row_id+"&backUrl=suchleiste_dessert.php";
        }
    </script>
...
<div class="all">
    <div class="main_wrapper">
        ...
        $search = $_POST["search"];
        $searchnew = "%$search%";
        $anzahl = $pdo->prepare("SELECT COUNT(*) FROM recipe, recipe_dish_type WHERE title LIKE :searchnew AND recipe_dish_type.dish_type_id='2' AND recipe.id=recipe_id ORDER BY RAND()");
        ...
        <div class="container_mittig">
            <div class="weite_suchleiste">
                <form action="suchleiste_do_dessert.php" method="post" enctype="multipart/form-data">
                    <div class="input-group mb-3">
                        <div class="input-group-prepend">
                            <button class="btn btn-light dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="height: 53px;margin-top: 1px;color: #FC7F16">Dessert</button>
        ...
        <!--AUSGABE ERGEBNISSE-->
        <?php
        $statement=$pdo->prepare("SELECT recipe.id, title, description, level_name, timespan, filename, dish_type_id FROM recipe,level,timespan, picture, recipe_dish_type, dish_type WHERE recipe_dish_type.dish_type_id='2' AND title LIKE :searchnew AND recipe_dish_type.dish_type_id=dish_type.id AND recipe_dish_type.recipe_id=recipe.id  AND recipe.level_id=level.id AND recipe.timespan_id=timespan.id AND picture.recipe_id=recipe.id GROUP BY recipe.id ORDER BY recipe.id DESC");
